...
#include "stm32f4xx_hal.h"   // 必须加！
#include "stdio.h"
#include "lcd_spi_154.h"


extern SPI_HandleTypeDef hspi3;   // 使用CubeMX定义的SPI句柄
...
*	入口参数:	无
*	返 回 值:	无
*	函数功能:	初始化LCD专用GPIO引脚（CS、DC、背光）
*	说    明:	SPI引脚由CubeMX生成的spi.c初始化		
*************************************************************************************************/
...
void LCD_GPIO_Init(void)
{
   ...
   GPIO_LCD_DC_CLK;          		// 使能 数据指令选择 引脚时钟
	...
	// 初始化片选引脚
	GPIO_InitStruct.Pin 		= LCD_CS_PIN;						// CS 引脚
...
{
   LCD_GPIO_Init();              // 初始化LCD专用GPIO引脚(CS、DC、背光)
   
   // 配置SPI为单线发送模式（LCD只需要发送数据）
